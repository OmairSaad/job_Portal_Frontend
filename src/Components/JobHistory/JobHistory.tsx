import { Tabs, TabsList } from "@mantine/core";
import HistoryCard from "./HistoryCard";
import { RootState } from "../../Store";
import { useDispatch, useSelector } from "react-redux";
import { useEffect } from "react";
import { Job } from "../../Interfaces/PostJob";
import { getAllJobs } from "../../Slices/JobSlice";

const JobHistory = () => {
    const dispatch = useDispatch();
    const { jobs } = useSelector((state:RootState) => state.jobs);
    const user = useSelector((state:RootState)=>state.user);
    // const [jobs,setJobs] = useState([]);
    useEffect(()=>{
    //    fetchJobs();
      dispatch(getAllJobs({userId:user.id}));
      console.log(jobs);
    },[dispatch])
    // const fetchJobs = async ()=>{
    //   getAllJobs().then((res)=>{
    //     console.log(res.data);
    //     // setJobs(res.data);
    //   })
    // }
    return (
        <div>
            <div className="text-2xl font-semibold mb-5 pt-5">Job History</div>
            <Tabs variant="outline" radius="lg" defaultValue="applied">
                <TabsList className=" [&_button]:font-semibold [&_button]:!text-lg mb-5 [&_button[data-active='true']]:text-bright-sun-400">
                    <Tabs.Tab value="applied">Applied</Tabs.Tab>
                    <Tabs.Tab value="saved">Saved</Tabs.Tab>
                    <Tabs.Tab value="offered">Offered</Tabs.Tab>
                    <Tabs.Tab value="interviewing">Interviewing</Tabs.Tab>
                </TabsList>

                <Tabs.Panel value="applied">
                    <div className="flex flex-wrap gap-5 mt-5">
                        {
                            jobs.map((job)=>{
                               return job.applicationStatus && <HistoryCard jobdetails={job} applied={true} saved={false} offered={false} interviewing={false} timestamp={job.applicantTimestamp}/>
                            })
                        }
                    </div>
                </Tabs.Panel>

                <Tabs.Panel value="saved">
                    <div className="flex flex-wrap gap-5 mt-5">
                        {
                            jobs.map((job) => {
                                return (
                                   job.saved && <HistoryCard jobdetails={job} applied={false} saved={true} offered={false} interviewing={false} timestamp={job.postedAgo}/>
                                )
                            })
                        }
                    </div>                
                </Tabs.Panel>

                <Tabs.Panel value="offered">
                    <div className="flex flex-wrap gap-5 mt-5">
                        {
                            jobs.map((job) => {
                                return (
                                    <HistoryCard jobdetails={job} applied={false} saved={false} offered={true} interviewing={false} timestamp={job.applicantTimestamp} />
                                )
                            })
                        }
                    </div>                
                </Tabs.Panel>

                <Tabs.Panel value="interviewing">
                    <div className="flex flex-wrap gap-5 mt-5">
                        {
                            jobs.map((job) => {
                                return (
                                    <HistoryCard jobdetails={job} applied={false} saved={false} offered={false} interviewing={true} timestamp={job.applicantTimestamp}/>
                                )
                            })
                        }
                    </div>
                </Tabs.Panel>
            </Tabs>
        </div>
    )
}
export default JobHistory;